...
// Layout dengan Tema Earth Tone Hijau Sage Adem (Soft Sage Green & Warm Cream)
...
<body class="bg-[#f5f7f4] text-[#363d34] min-h-screen flex flex-col font-sans">
    <nav class="bg-[#586b55] text-white shadow-md">
        <div class="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
            <a href="/dashboard" class="font-bold text-xl flex items-center space-x-2"><span>🌿</span><span>Portal Walimurid Kelas 2A</span></a>
            <a href="/logout" class="bg-[#475644] hover:bg-[#384435] px-4 py-2 rounded-xl text-sm font-medium transition shadow-sm">Keluar</a>
        </div>
    </nav>
...
app.get('/login', (req, res) => {
    ...
    <!DOCTYPE html>
    <html lang="id">
    <head><meta charset="UTF-8"><title>Login - Portal Walimurid Kelas 2A</title><script src="https://cdn.tailwindcss.com"></script></head>
    <body class="bg-[#f5f7f4] flex items-center justify-center min-h-screen">
        <div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-[#d8ded5]">
            <div class="text-center mb-6">
                <h1 class="text-2xl font-bold text-[#586b55] mt-2">Portal Walimurid Kelas 2A</h1>
                <p class="text-sm text-[#717d6e] mt-1">Input nama depan ananda sebagai username dan password</p>
            </div>
            <form action="/login" method="POST" class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-[#475644] mb-1">Username</label>
                    <input type="text" name="first_name" required class="w-full px-4 py-2 border border-[#cbd5c8] rounded-xl focus:ring-2 focus:ring-[#586b55] outline-none transition" placeholder="Input nama depan ananda">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-[#475644] mb-1">Password</label>
                    <input type="password" name="password" required class="w-full px-4 py-2 border border-[#cbd5c8] rounded-xl focus:ring-2 focus:ring-[#586b55] outline-none transition" placeholder="Input password">
                </div>
                <button type="submit" class="w-full bg-[#586b55] text-white py-3 rounded-xl font-bold hover:bg-[#475644] shadow-md transition">Masuk</button>
            </form>
        </div>
    </body></html>`);
});
...
app.get('/dashboard', checkAuth, (req, res) => {
    ...
    <div class="mb-8 bg-gradient-to-r from-[#586b55] to-[#73876f] text-white p-6 rounded-2xl shadow-md flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-bold">Assalamualaikum, Ayah/Bunda Ananda ${String(req.user.first_name)}</h2>
        </div>
        <div class="text-4xl hidden sm:block">🌿</div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <a href="/calendar" class="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition border border-[#d8ded5] flex items-center space-x-5 group">
            <div class="bg-[#f0f4ee] text-[#586b55] p-4 rounded-xl text-3xl group-hover:bg-[#586b55] group-hover:text-white transition">📅</div>
            <div>
                <h3 class="font-bold text-lg text-[#363d34] group-hover:text-[#586b55] transition">Kalendar Akademik</h3>
                <p class="text-sm text-[#717d6e]">Agenda kelas dan catatan jadwal pribadi ananda.</p>
            </div>
        </a>
        <a href="/kas" class="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition border border-[#d8ded5] flex items-center space-x-5 group">
            <div class="bg-[#f0f4ee] text-[#586b55] p-4 rounded-xl text-3xl group-hover:bg-[#586b55] group-hover:text-white transition">💵</div>
            <div>
                <h3 class="font-bold text-lg text-[#363d34] group-hover:text-[#586b55] transition">Iuran Kas Siswa</h3>
                <p class="text-sm text-[#717d6e]">Pembayaran kas pribadi setiap siswa.</p>
            </div>
        </a>
        <a href="/finances" class="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition border border-[#d8ded5] flex items-center space-x-5 group">
            <div class="bg-[#fcf8f2] text-[#b8860b] p-4 rounded-xl text-3xl group-hover:bg-[#b8860b] group-hover:text-white transition">📊</div>
            <div>
                <h3 class="font-bold text-lg text-[#363d34] group-hover:text-[#b8860b] transition">Laporan Keuangan</h3>
                <p class="text-sm text-[#717d6e]">Laporan income & expense kelas 2A Tahun Ajaran 2026/2027.</p>
            </div>
        </a>
        <a href="/announcements" class="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition border border-[#d8ded5] flex items-center space-x-5 group">
            <div class="bg-[#f0f4ef] text-[#31708f] p-4 rounded-xl text-3xl group-hover:bg-[#31708f] group-hover:text-white transition">📢</div>
            <div>
                <h3 class="font-bold text-lg text-[#363d34] group-hover:text-[#31708f] transition">Pengumuman</h3>
                <p class="text-sm text-[#717d6e]">Pengumuman dari pihak sekolah.</p>
            </div>
        </a>
    </div>`;
...
app.get('/calendar', checkAuth, async (req, res) => {
    try {
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-[#d8ded5]">
            <div>
                <h2 class="text-2xl font-bold text-[#363d34]">Kalendar Akademik Tahun Ajaran 2026/2027</h2>
                <p class="text-sm text-[#717d6e]">Agenda kelas dan catatan jadwal pribadi siswa/siswi.</p>
            </div>
        ...
        <form action="/calendar/save" method="POST">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-[#d8ded5] p-6">
            ...
            <div class="mt-6 flex justify-end">
                <button type="submit" class="bg-[#586b55] hover:bg-[#475644] text-white px-8 py-3 rounded-xl font-bold text-sm shadow-md transition">💾 Simpan jadwal pribadi ananda</button>
            </div>
        </form>
...
app.get('/kas', checkAuth, async (req, res) => {
    try {
        ...
        const content = `
        <div class="mb-6"><h2 class="text-2xl font-bold text-[#363d34]">Iuran Kas Siswa</h2><p class="text-sm text-[#717d6e]">Track pembayaran iuran kas kelas ananda.</p></div>
...
app.get('/finances', checkAuth, async (req, res) => {
    try {
        ...
        db.transactions.forEach(tx => {
            ...
            rows += `<tr class="border-b border-[#d8ded5] hover:bg-[#f0f2ef] transition"><td class="py-4 px-6 text-sm text-[#717d6e]">${tx.date}</td><td class="py-4 px-6 font-medium text-[#363d34]">${descText}</td><td class="py-4 px-6">${badge}</td><td class="py-4 px-6 font-bold text-[#363d34]">Rp ${amt.toLocaleString()}</td></tr>`;
        });
        
        const balance = totalIncome - totalExpense;
        const content = `
        <div class="mb-6"><h2 class="text-2xl font-bold text-[#363d34]">Laporan Keuangan</h2><p class="text-sm text-[#717d6e]">Laporan income & expense kelas 2A 2026/2027.</p></div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#d8ded5]"><span class="text-xs font-bold uppercase tracking-wider text-[#717d6e]">Total Income</span><h3 class="text-2xl font-black text-[#2e6930] mt-1">Rp ${totalIncome.toLocaleString()}</h3></div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#d8ded5]"><span class="text-xs font-bold uppercase tracking-wider text-[#717d6e]">Total Expenses</span><h3 class="text-2xl font-black text-[#b33a3a] mt-1">Rp ${totalExpense.toLocaleString()}</h3></div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#d8ded5]"><span class="text-xs font-bold uppercase tracking-wider text-[#717d6e]">Current Balance</span><h3 class="text-2xl font-black text-[#586b55] mt-1">Rp ${balance.toLocaleString()}</h3></div>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-[#d8ded5] overflow-hidden">
            <table class="w-full text-left">
    ...
    } catch (e) { res.status(500).save("Error"); }
});
...
app.get('/announcements', checkAuth, async (req, res) => {
    try {
        ...
        db.announcements.forEach(a => {
            let imageHtml = '';
            // Mengatasi gambar pecah: hanya merender tag img jika link diisi dengan benar dan berformat link gambar langsung
            ...
            cards += `
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-[#d8ded5] mb-4">
                <div class="flex justify-between items-center mb-2">
                    <h3 class="font-bold text-lg text-[#586b55]">${a.title}</h3>
                    <span class="text-xs font-medium bg-[#f0f2ef] text-[#717d6e] px-3 py-1 rounded-full border border-[#d8ded5]">${a.date}</span>
                </div>
                <p class="text-[#475644] text-sm leading-relaxed">${a.content}</p>
                ${imageHtml}
            </div>`;
        });
        const content = `
        <div class="mb-6"><h2 class="text-2xl font-bold text-[#363d34]">Pengumuman</h2><p class="text-sm text-[#717d6e]">Pengumuman dari pihak sekolah.</p></div>
        <div>${cards}</div>
        <div class="mt-6"><a href="/dashboard" class="inline-flex items-center text-[#586b55] hover:text-[#363d34] text-sm font-semibold">&larr; Kembali ke Beranda</a></div>`;
